...
## V3.6 Hardened Patch
- Master shortlist now has a stable schema even when zero candidates qualify.
- Empty Master is treated as a valid research outcome, not an application error.
- Downstream portfolio/forward-ledger code safely handles empty or schema-mismatched Master frames.
- Added non-raising Master schema audit helper.
- Replaced deprecated Streamlit `use_container_width` arguments with the current `width` API.
- R:R, entry, stop and position sizing remain manual and are not Master gates.
