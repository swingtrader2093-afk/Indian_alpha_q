...
MASTER_COLUMNS = [
    ...
    "Breakout/trigger","Volume confirmation","Master status","As of"
]
...
def build_master_shortlist(res, pattern_df):
    """Build an explainable, schema-stable current Master shortlist.

    Empty Master is a normal research outcome, never an application error.
    R:R/entry/stop/position sizing are deliberately NOT gates.
    """
    ...
    if r.empty or "Current signal active" not in r.columns:
        return empty_master_frame(), r
    r=r[r["Current signal active"].fillna(False).astype(bool)].copy()
    if r.empty or pattern_df is None or not isinstance(pattern_df,pd.DataFrame) or pattern_df.empty or "Ticker" not in pattern_df.columns:
        return empty_master_frame(), r
    p=pattern_df.copy()
    ...
    keep=[c for c in ["Ticker","Pattern","Pattern score","Breakout/trigger","Volume confirmation","Last price","As of"] if c in p.columns]
    if "Ticker" not in keep:
        return empty_master_frame(), r
    m=r.merge(p[keep],on="Ticker",how="inner")
    if m.empty:
        return empty_master_frame(), r
    # Always materialize the downstream-required field before any filtering.
    if "Volume confirmation" not in m.columns:
        m["Volume confirmation"]=""
    vc=m["Volume confirmation"].fillna("").astype(str).str.upper()
    m["Master status"]=np.where(vc.eq("PASS"),"MASTER","SETUP WATCH")
    m=m[m["Master status"].eq("MASTER")].copy()
    # Stable output schema: add absent optional columns as blank values.
    for c in MASTER_COLUMNS:
        if c not in m.columns:
            m[c]=np.nan if c not in ("Master status","As of") else ("" if c=="Master status" else "")
    sort_cols=[c for c in ["Research score","Pattern score","WF Sharpe"] if c in m.columns]
    if sort_cols and not m.empty:
        m=m.sort_values(sort_cols,ascending=[False]*len(sort_cols))
    return m[MASTER_COLUMNS],r
...
if os.environ.get("QUANT_WORKER") != "1":
    ...
    with tab_master:
        ...
        st.caption("The decision view: the SAME strategy must have strong historical evidence, be active on the latest bar, have a current chart pattern, and have volume confirmation. Risk/reward is deliberately left to your manual process.")
        ...
        st.markdown("**Flow:** NSE universe → fast screen → historical validation → walk-forward/robustness → SAME strategy active now → current pattern → volume → ⭐ Master Shortlist → your chart review")
    ...
    with tab_opportunity:
        ...
        st.caption("Historical quantitative evidence + current pattern. Use this for investigation; the Master tab applies the stricter SAME-strategy-current-signal gate.")
            ...
            if payload:
                ...
                if not valid.empty:
                    ...
                    if master.empty:
                        ...
                        master_cols=[c for c in ["Ticker","Strategy","Research score","WF Sharpe","WF PF","Verdict","Current signal active","Current price","Current 52w high distance","Current trend","Current volume ratio","Pattern","Pattern score","Breakout/trigger","Volume confirmation","Master status","As of"] if c in master.columns]
