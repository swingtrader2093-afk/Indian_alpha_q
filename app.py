...
def worker_main(job_id):
    ...
    try:
        ...
        portfolio_df,portfolio_meta=portfolio_context(master[master["Master status"].eq("MASTER")].copy()) if not master.empty else (pd.DataFrame(),{})
        ...
        record_master_signals(master[master["Master status"].eq("MASTER")].copy(),job.name)
...
def build_master_shortlist(res, pattern_df):
    """Build an explainable current master shortlist.

    A row qualifies only when the SAME strategy that has historical evidence
    is active on the latest bar, the historical result is PROMISING, and a
    current classical pattern is present. This is intentionally a gate-based
    shortlist, not a new opaque composite score.
    """
    if res is None or res.empty or "Verdict" not in res.columns:
        return pd.DataFrame(), pd.DataFrame()
    r=res[res["Verdict"].eq("PROMISING")].copy()
    if r.empty or "Current signal active" not in r.columns:
        return pd.DataFrame(), pd.DataFrame()
    r=r[r["Current signal active"].fillna(False)].copy()
    if r.empty or pattern_df is None or pattern_df.empty or "Ticker" not in pattern_df.columns:
        return pd.DataFrame(), r
    p=pattern_df.copy()
    p["Ticker"]=p["Ticker"].astype(str)
    # Keep the strongest current pattern for each ticker; pattern score is
    # descriptive structural fit, not probability.
    ...
    if m.empty:
        return m, r
    # Strict Master is intentionally small: volume confirmation is the final
    # confirmation gate. Developing setups remain available in Opportunity.
    vc=m.get("Volume confirmation",pd.Series("",index=m.index)).astype(str).str.upper()
...
if os.environ.get("QUANT_WORKER") != "1":
    ...
    def show_candidate_evidence(payload, evidence_key):
        ...
        st.dataframe(pd.DataFrame([r0[btcols].to_dict()]),use_container_width=True,hide_index=True)

        with st.expander("Trade log",expanded=False):
            t0=d0.get("trades",pd.DataFrame())
            st.dataframe(t0,use_container_width=True,hide_index=True)
        with st.expander("Walk-forward folds",expanded=False):
            f0=d0.get("wf_folds",pd.DataFrame())
            st.dataframe(f0,use_container_width=True,hide_index=True)
        with st.expander("Walk-forward trades",expanded=False):
            wt0=d0.get("wf_trades",pd.DataFrame())
            st.dataframe(wt0,use_container_width=True,hide_index=True)
        with st.expander("Parameter sensitivity",expanded=False):
            se0=d0.get("sensitivity",pd.DataFrame())
            st.dataframe(se0,use_container_width=True,hide_index=True)
                ...
                if not rr.empty and not pp.empty:
                    ...
                    if not rv.empty:
                        ...
                        if not combo.empty:
                            ...
                            st.dataframe(combo.head(100),use_container_width=True)
            ...
            if payload:
                ...
                if pattern_file.exists():
                    try:
                        ...
                        if not auto_pr.empty:
                            ...
                            st.dataframe(auto_pr.head(100),use_container_width=True)
                    except Exception:
                        pass
                ...
                if not valid.empty:
                    ...
                    if master.empty:
                        ...
                        st.dataframe(master[master_cols],use_container_width=True,hide_index=True)
                        ...
                        if payload_for_evidence:
                            for ei,(_,erow) in enumerate(master.head(20).iterrows()):
                                ...
                                if bc2.button("🔬 Inspect",key=f"inspect_master_{ei}",use_container_width=True):
                ...
                if ms_path.exists():
                    try:
                        ...
                        st.dataframe(ml.tail(100),use_container_width=True,hide_index=True)
                    except Exception: pass
